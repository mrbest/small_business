...
sc <- sparkInit()
tic()
load_spark_csv(sc, "../../GWCM_Extracts/GWCM_FPDS_FY13_PRESENT_DEC_28.TSV", "../GWCM_Exports/RAW_29DEC17.prq", "../GWCM_Exports/GWCM_29DEC17.prq")
toc()
tic()
create_mini_from_df(raw_df, "../GWCM_Exports/MINI_RAW_29DEC17.prq")
toc()
...
load_spark_parquet("../GWCM_Exports/RAW_29DEC17.prq/")
toc()
new_df <- create_mini(raw_df)
spark_write_parquet(new_df, "RAW_29DEC17Mini.prq")
```
...
toc()
tic()
set_pre_sb_exclusions_df(fy16trans)
sb_exclusions_mod <- fy16trans %>% mutate(sb_exclude = designate_small_business_eligble(sheltered_workshop_flag,
                                                                                                                 pop_country_code,
                                                                                                                 date_signed,
                                                                                                                 vendor_duns_number,
                                                                                                                 contingency_ops,
...
source("small_business.R")
tic()
sb_exclusions_mod <- fy16trans %>% rowwise() %>% 
  mutate(sb_exclude = designate_small_business_eligble(sheltered_workshop_flag, pop_country_code, date_signed, vendor_duns_number, contingency_ops, foreign_government, foreign_funding_desc, funding_agency_code, reason_not_competed, product_or_service_code, contracting_office_id,  piid, reference_piid, `idv_ref_idv_piid) )
toc()
